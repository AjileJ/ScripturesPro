...
export const loadScriptures = (id) => dispatch => {
    ...
    axiosWithAuth()
        ...
        .then(res => {
            dispatch({
                type: LOADSCRIPTURES_SUCCESS, payload: res.data.scriptures})
        })
        .catch(err => {
            dispatch({
                type: LOADSCRIPTURES_FAIL, payload: err})
        })
}
...
export const addScriptures = (id, newScripture) => dispatch => {
    ...
    axiosWithAuth()
        ...
        .then(res => {
            dispatch({type: ADDSCRIPTURES_SUCCESS, payload: res.data.scriptures})
        })
        .catch(err => {
            dispatch({
                type: ADDSCRIPTURES_FAIL, payload: err})
        })
}
...
export const deleteScripture = (user_id, scripture_id) => dispatch => {
  ...
  export const updateScripture = (user_id, scripture_id, scripture) => dispatch => {
    ...
    axiosWithAuth()
        ...
        .then(res => {
        dispatch({
            type: EDITSCRIPTURES_SUCCESS, payload: res.data.scriptures})
        })
        .catch(err => {
            dispatch({
                type: EDITSCRIPTURES_FAIL, payload: err})
        })
  }
